...
// US Format: ^[AKNW][A-Z]{0,2}[0-9][A-Z]{1,3}$
// Examples: W1ABC, K4XYZ, N2QWE, KA1ABC, WB4XYZ, etc.
String generateCallsign() {
  ...
  // Optional: 0-2 additional prefix letters
  int extraPrefixCount = random(0, 3); // 0, 1, or 2 additional letters
  for (int i = 0; i < extraPrefixCount; i++) {
...
void drawHearItTypeItUI(Adafruit_ST7789& tft) {
  ...
  tft.setTextColor(0x7BEF);
  tft.setTextSize(1);
  String helpText = "ENTER Submit  ESC Replay  TAB Skip";
...
int handleHearItTypeItInput(char key, Adafruit_ST7789& tft) {
  if (!waitingForInput && key != KEY_ESC && key != KEY_TAB) {
